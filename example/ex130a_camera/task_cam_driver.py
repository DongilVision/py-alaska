...
class imi_cam_dp:
    ...
    def _session_open(self):
        ...
        if self.handle:
            ...
            print(f"{TAG} Session open (acquisition stopped)")
    ...
    def _session_close(self):
        ...
        if self.handle:
            ...
            print(f"{TAG} Session close (acquisition started)")
    ...
    def _hw_set_fps(self, value):
        ...
        if err != 0:
            print(f"{TAG} ERROR ntcSetFrameRate failed: fps={value}, err={err}")
        else:
            print(f"{TAG} FPS set to {value}")

    def _hw_set_exposure(self, value):
        pui = ctypes.c_uint32(value)
        if ntcSetExposureTime(self.handle, pui) != 0:
            print(f"{TAG} ERROR ntcSetExposureTime failed")
        else:
            print(f"{TAG} Exposure set to {value}")
    ...
    def _hw_set_trigger(self, enabled):
        ...
        if ntcSetTrigger(self.handle, st_trigger) != ENeptuneError.NEPTUNE_ERR_Success.value:
            print(f"{TAG} ERROR ntcSetTrigger failed")
        else:
            print(f"{TAG} Trigger set to {enabled}")
    ...
    def re_connect(self):
        ...
        if not imi_cam_dp.DriverInit:
            if ntcInit() != ENeptuneError.NEPTUNE_ERR_Success.value:
                print(f"{TAG} ERROR Driver initialization failed")
                return False
            imi_cam_dp.DriverInit = True
            print(f"{TAG} Driver initialized")
        ...
        if err != ENeptuneError.NEPTUNE_ERR_Success.value:
            print(f"{TAG} ERROR ntcOpen failed: {self.mac_address} (err={err})")
            return False

        if ntcSetHeartbeatTime(self.handle, HEARTBEAT_TIME) != ENeptuneError.NEPTUNE_ERR_Success.value:
            print(f"{TAG} ERROR ntcSetHeartbeatTime failed")
            return False
    ...
    def close(self):
        print(f"{TAG} Close called, connected={self.is_connect}")
        ...
        try:
            ...
            print(f"{TAG} ERROR during close: {e}")
    ...
    def run(self):
        ...
        while not self.runtime.should_stop():
            try:
                ...
                action = cmd["action"]
                source = cmd["source"]
                print(f"{TAG} cmd: {action} (from {source})")
            ...
            if self.is_connect:
                ...
                if disconnect_counter >= wait_count:
                    if (disconnect_counter - wait_count) % log_interval == 0:
                        ...
                        print(f"{TAG} reconnecting... ({elapsed}s)")
                    self.re_connect()

        self._emit_disconnected("end_of_run")
        print(f"{TAG} run stopped")

    # ═══════════════════════════════════════════════════════════════════════
    # Hardware Callbacks
    # ═══════════════════════════════════════════════════════════════════════
    def RecvDeviceCheckCallBack(self, state, pContext=None):
        print(f"{TAG} Device {'added' if state == 0 else 'removed'}")
        if state == 0:
    ...
    def RecvTimeoutCallBack(self, _=None):
        print(f"{TAG} WARN Frame timeout")

    def RecvFrameCallBack(self, pImage, pContext=None):
        self.rx_count += 1

        if self.smblock is None:
            print(f"{TAG} WARN SmBlock not initialized")
            return
        ...
        try:
            ...
            if sm_channels == 1:
                if pImage.uiBitDepth != 8:
                    ...
                    cv2.cvtColor(
                        rx_frame.reshape(pImage.uiHeight, pImage.uiWidth, -1),
                        cv2.COLOR_BGR2GRAY, dst=dst_buffer)
            else:
                if pImage.uiBitDepth != 8:
                    cv2.cvtColor(
                        rx_frame.reshape(pImage.uiHeight, pImage.uiWidth),
            ...
            self.smblock.mfree(index)
            print(f"{TAG} ERROR Frame processing error: {e}")
    ...
    def one_shot(self):
        if not self.is_connect:
            print(f"{TAG} WARN one_shot called but not connected")
            return
        if self.trigger_source != "software":
            print(f"{TAG} WARN one_shot: trigger_source is not 'software'")
            return
        err = ntcRunSWTrigger(self.handle)
        if err != ENeptuneError.NEPTUNE_ERR_Success.value:
            print(f"{TAG} ERROR ntcRunSWTrigger failed (err={err})")
    ...
    def _log_discovery_table(self, mac_list, result, status):
        ...
        print(f"{TAG} Discovery: {len(mac_list)} camera(s), target={result}, status={status}")
